...
public interface ICompanyService
{
    ...
    Task<IReadOnlyList<CompanyResponseDto>> GetByOwnerAsync(
        Guid userId);
    ...
    Task<CompanyDocumentResponseDto> UploadDocumentAsync(
        Guid companyId,
        Guid userId,
        UploadCompanyDocumentDto dto);

    Task<CompanyVerificationStatusDto> SubmitVerificationAsync(
        Guid companyId,
        Guid userId,
        SubmitCompanyVerificationDto dto);

    Task<CompanyVerificationStatusDto> GetVerificationStatusAsync(
        Guid companyId,
        Guid userId,
        bool isAdmin = false);

    Task<CompanyResponseDto> VerifyCompanyAsync(
        Guid companyId,
        Guid adminUserId,
